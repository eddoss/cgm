...
/* ####################################################################################### */
    ...
    #define CGM_VERIFY_MATRIX_DIRECT_ITERATOR_DEREFERENCE(current, begin, end, message) \
    if (current >= end || current < begin) {throw std::range_error(message);}

/* --------------------------------------------------------------------------------------- */

    #define CGM_VERIFY_MATRIX_DIRECT_ITERATOR_SELECTOR(current, begin, end, message) \
    if (current >= end || current < begin) {throw std::range_error(message);}

/* --------------------------------------------------------------------------------------- */

    #define CGM_VERIFY_MATRIX_DIRECT_ITERATOR_FORWARD(current, end, message) \
    if (current > end) {throw std::range_error(message);}

/* --------------------------------------------------------------------------------------- */

    #define CGM_VERIFY_MATRIX_DIRECT_ITERATOR_BACKWARD(current, begin, message) \
    if (current < begin) {throw std::range_error(message);}

/* --------------------------------------------------------------------------------------- */

    #define CGM_VERIFY_MATRIX_DIRECT_ITERATOR_GET_ROW(current, begin, end, message) \
    if (current >= end || current < begin) {throw std::range_error(message);}

/* --------------------------------------------------------------------------------------- */

    #define CGM_VERIFY_MATRIX_DIRECT_ITERATOR_GET_COLUMN(current, begin, end, message) \
    if (current >= end || current < begin) {throw std::range_error(message);}
...
/* ####################################################################################### */
    ...
    operator*() const
    {
        CGM_VERIFY_MATRIX_DIRECT_ITERATOR_DEREFERENCE(m_data, m_begin, m_begin + M*N, "(CGM) can't dereference out of range matrix iterator.")
        return *m_data;
    }
...
/* --------------------------------------------------------------------------------------- */
    ...
    operator->() const
    {
        CGM_VERIFY_MATRIX_DIRECT_ITERATOR_SELECTOR(m_data, m_begin, m_begin + M*N, "(CGM) can't call selector of out of range matrix iterator.")
        return m_data;
    }
...
/* ####################################################################################### */
    ...
    {
        ++m_data;
        CGM_VERIFY_MATRIX_DIRECT_ITERATOR_FORWARD(m_data, m_begin + M*N, "(CGM) can't pre-increment matrix iterator after end.")
        return *this;
    }
...
/* --------------------------------------------------------------------------------------- */
    ...
    operator++(int)
    {
        self_type tmp = *this;
        ++m_data;
        CGM_VERIFY_MATRIX_DIRECT_ITERATOR_FORWARD(m_data, m_begin + M*N, "(CGM) can't post-increment matrix iterator after end.")
        return tmp;
    }
...
/* --------------------------------------------------------------------------------------- */
    ...
    {
        m_data += offset;
        CGM_VERIFY_MATRIX_DIRECT_ITERATOR_FORWARD(m_data, m_begin + M*N, "(CGM) can't move matrix iterator forward after end.")
        return *this;
    }
...
/* ####################################################################################### */
    ...
    {
        --m_data;
        CGM_VERIFY_MATRIX_DIRECT_ITERATOR_BACKWARD(m_data, m_begin, "(CGM) can't pre-decrement matrix iterator before begin.")
        return *this;
    }
...
/* --------------------------------------------------------------------------------------- */
    ...
    operator--(int)
    {
        self_type tmp = *this;
        --m_data;
        CGM_VERIFY_MATRIX_DIRECT_ITERATOR_BACKWARD(m_data, m_begin, "(CGM) can't post-decrement matrix iterator before begin.")
        return tmp;
    }
...
/* --------------------------------------------------------------------------------------- */
    ...
    {
        m_data -= offset;
        CGM_VERIFY_MATRIX_DIRECT_ITERATOR_BACKWARD(m_data, m_begin, "(CGM) can't move matrix iterator backward before begin.")
        return *this;
    }
