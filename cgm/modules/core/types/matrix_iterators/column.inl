...
/* ####################################################################################### */
    ...
    #define CGM_DETAIL_VERIFY_MATRIX_COLUMN_ITERATOR_DEREFERENCE(current, begin, end, message) \
    if (current >= end || current < begin) {throw std::range_error(message);}

/* --------------------------------------------------------------------------------------- */

    #define CGM_DETAIL_VERIFY_MATRIX_COLUMN_ITERATOR_SELECTOR(current, begin, end, message) \
    if (current >= end || current < begin) {throw std::range_error(message);}

/* --------------------------------------------------------------------------------------- */

    #define CGM_DETAIL_VERIFY_MATRIX_COLUMN_ITERATOR_FORWARD(current, end, message) \
    if (current > end) {throw std::range_error(message);}

/* --------------------------------------------------------------------------------------- */

    #define CGM_DETAIL_VERIFY_MATRIX_COLUMN_ITERATOR_BACKWARD(current, begin, message) \
    if (current < begin) {throw std::range_error(message);}

/* --------------------------------------------------------------------------------------- */

    #define CGM_DETAIL_VERIFY_MATRIX_COLUMN_ITERATOR_GET_ROW(current, begin, end, message) \
    if (current >= end || current < begin) {throw std::range_error(message);}

/* --------------------------------------------------------------------------------------- */

    #define CGM_DETAIL_VERIFY_MATRIX_COLUMN_ITERATOR_GET_COLUMN(current, begin, end, message) \
    if (current >= end || current < begin) {throw std::range_error(message);}
...
/* ####################################################################################### */
    ...
    operator*() const
    {
        CGM_DETAIL_VERIFY_MATRIX_COLUMN_ITERATOR_DEREFERENCE(m_row, 1, M + 1, "(CGM) can't dereference out of range matrix iterator.")
        return *m_data;
    }
...
/* --------------------------------------------------------------------------------------- */
    ...
    operator->() const
    {
        CGM_DETAIL_VERIFY_MATRIX_COLUMN_ITERATOR_SELECTOR(m_row, 1, M + 1, "(CGM) can't call selector of out of range matrix iterator.")
        return m_data;
    }
...
/* ####################################################################################### */
    ...
    operator++()
    {
        ++m_row;
        recalculateDataAddress();
        CGM_DETAIL_VERIFY_MATRIX_COLUMN_ITERATOR_FORWARD(m_row, M + 1, "(CGM) can't pre-increment matrix iterator after end.")
        return *this;
    }
...
/* --------------------------------------------------------------------------------------- */
    ...
    operator++(int)
    {
        ...
        ++m_row;
        recalculateDataAddress();
        CGM_DETAIL_VERIFY_MATRIX_COLUMN_ITERATOR_FORWARD(m_row, M + 1, "(CGM) can't post-increment matrix iterator after end.")
        return tmp;
    }
...
/* --------------------------------------------------------------------------------------- */
    ...
    operator+=(difference_type offset)
    {
        m_row += offset;
        recalculateDataAddress();
        CGM_DETAIL_VERIFY_MATRIX_COLUMN_ITERATOR_FORWARD(m_row, M + 1, "(CGM) can't move matrix iterator forward after end.")
        return *this;
    }
...
/* ####################################################################################### */
    ...
    operator--()
    {
        --m_row;
        recalculateDataAddress();
        CGM_DETAIL_VERIFY_MATRIX_COLUMN_ITERATOR_BACKWARD(m_row, 1, "(CGM) can't pre-decrement matrix iterator before begin.")
        return *this;
    }
...
/* --------------------------------------------------------------------------------------- */
    ...
    operator--(int)
    {
        ...
        --m_row;
        recalculateDataAddress();
        CGM_DETAIL_VERIFY_MATRIX_COLUMN_ITERATOR_BACKWARD(m_row, 1, "(CGM) can't post-decrement matrix iterator before begin.")
        return tmp;
    }
...
/* --------------------------------------------------------------------------------------- */
    ...
    operator-=(difference_type offset)
    {
        m_row -= offset;
        recalculateDataAddress();
        CGM_DETAIL_VERIFY_MATRIX_COLUMN_ITERATOR_BACKWARD(m_row, 1, "(CGM) can't move matrix iterator backward before begin.")
        return *this;
    }
...
/* ####################################################################################### */
    ...
    row() const
    {
        CGM_DETAIL_VERIFY_MATRIX_COLUMN_ITERATOR_GET_ROW(m_row, 1, M + 1, "(CGM) can't get row from matrix iterator (out of range).")
        return m_row - 1;
    }
...
/* --------------------------------------------------------------------------------------- */
    ...
    column() const
    {
        CGM_DETAIL_VERIFY_MATRIX_COLUMN_ITERATOR_GET_COLUMN(m_row, 1, M + 1, "(CGM) can't get column from matrix iterator (out of range).")
        return m_column;
    }
