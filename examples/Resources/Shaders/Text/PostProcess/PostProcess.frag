...
vec3 getTexel(sampler2D image, vec2 coord)
{
    ...
    float value = (samples01.x + samples01.y + samples23.x + samples23.y + samples45.x + samples45.y) / 6.0f;
...
void main()
{
    if (enableSPAA)
    {
        ...
        vec3 c = getTexel(screenTexture, uv);           // current pixel
        vec3 l = getTexel(screenTexture, uv - step);    // left pixel
        vec3 r = getTexel(screenTexture, uv + step);    // right pixel
