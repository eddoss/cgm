...
TEST(Cartesian_3D_Functions_Matrix4x4_Converters, LocalToLocal_Quat_Mat3WithPos)
{
    using namespace cgm_xyz_converters_tests_data;

    auto result = CGM_XYZ::converted(MAT4, A_QUAT, B_MAT3, B_P);
...
TEST(Cartesian_3D_Functions_Matrix4x4_Converters, LocalToLocal_Quat_QuatWithPos)
{
    using namespace cgm_xyz_converters_tests_data;

    auto result = CGM_XYZ::converted(MAT4, A_QUAT, B_QUAT, B_P);
    ASSERT_TRUE(CGM::eq(result, EXPECT_MAT4_A2B_MAT3_TO_MAT4, 0.0001));
}