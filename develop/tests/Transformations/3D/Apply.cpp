...
TEST(Transformations_Apply, Vector_Matrix3)
{
    ...
    const auto result = CGM_XFORM3D::applied(cgm_test::vector, cgm_test::apply::mat3);
...
TEST(Transformations_Apply, Vector_Matrix4)
{
    ...
    {
        const auto result = CGM_XFORM3D::applied<CGM_POINT>(cgm_test::vector, cgm_test::apply::mat4);
        const auto expect = Vector<3,double>{ -1.09577, 1.42554, 0.821156 };
        ASSERT_TRUE(CGM::eq(result, expect, 0.00001));
    }
    {
        const auto result = CGM_XFORM3D::applied<CGM_DIRECTION>(cgm_test::vector, cgm_test::apply::mat4);
        const auto expect = Vector<3,double>{ -1.39577, 2.62554, -0.178844 };
        ASSERT_TRUE(CGM::eq(result, expect, 0.00001));
    }
}
...
TEST(Transformations_Apply, Matrix3_Matrix3)
{
    ...
    const auto result = CGM_XFORM3D::applied(cgm_test::orientation, cgm_test::apply::mat3);
...
TEST(Transformations_Apply, Matrix4_Matrix4)
{
    ...
    const auto result = CGM_XFORM3D::applied(cgm_test::space, cgm_test::apply::mat4);
